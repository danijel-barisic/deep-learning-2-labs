...
class UMDist(MixtureDist):
    ...
    @classmethod
    def random(cls, K: int, a_range=(0, 1), b_range=(1, 2)):
        pi = np.random.dirichlet(5 * np.ones(K)).astype(np.float32)
        a = np.random.uniform(a_range[0], a_range[1], size=K).astype(np.float32)
        b = np.random.uniform(b_range[0], b_range[1], size=K).astype(np.float32)

        return cls(pi, a, b)

    def sample(self, L: int):
        choices = np.random.choice(np.arange(self.K), size=L, p=self.pi)
        sample = np.random.uniform(self.a[choices], self.b[choices], size=L).astype(np.float32)

        return sample

    @staticmethod
    def uniform_pdf(x: NumT, a: NumT, b: NumT):
        # Uniform PDF
        return (x >= a) * (x <= b) / (b - a)

    def p_xz(self, x: NumT, k: int):
